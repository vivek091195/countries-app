const MAX_VISIBLE_COUNTRIES = 16;
let startIndex = 0;
...
function formulateCountriesDropdown() {
  const selectElement = document.getElementById("region-dropdown");
...
function createCountryElements() {
  ...
  for (let i = 0; i < MAX_VISIBLE_COUNTRIES; i++) {
    const countryCard = createCountryCard();
    parentFragment.appendChild(countryCard);
  }

  document.getElementById("country-list").appendChild(parentFragment);
  updateCountryElementsWithData();
}
...
function createCountryCard() {
  ...
  const countryDetails = createCountryDetails();
...
function createCountryDetails() {
  ...
  const countryName = createCountryName();
  const detailsFragment = createCountryDetailFragment();

  countryDetails.appendChild(countryName);
  countryDetails.appendChild(detailsFragment);
...
function createCountryDetailFragment() {
  const fragment = document.createDocumentFragment();

  const population = createCountryDetail("Population");
  const region = createCountryDetail("Region");
  const capital = createCountryDetail("Capital");
...
function createCountryDetail(titleText) {
  ...
  countryDetail.appendChild(title);
  countryDetail.appendChild(value);
